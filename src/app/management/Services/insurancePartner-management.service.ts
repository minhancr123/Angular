...
import { InsurancePartnerModel } from '../Model/Inventory-management.model';
...
const API_INSURANCE_PARTNER = `${environment.ApiRoot}/InventoryManagement`;
...
export class InsurancePartnerService {
  ...
  private _items$ = new BehaviorSubject<InsurancePartnerModel[]>([]);
...
/**
  ...
  fetchForGuard(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.findData(this._tableState$.value)
        .pipe(
          tap((res: ResultModel<InsurancePartnerModel>) => {
            if (res && res.status === 1) {
  ...
  private findData(state: ITableState): Observable<any> {
    ...
    const url = `${API_INSURANCE_PARTNER}/GetAllDoitacBaohiem`;
    return this.http.post<any>(url, state, { headers });
  }

  createPartner(item: InsurancePartnerModel): Observable<any> {
    const url = `${API_INSURANCE_PARTNER}/addDoitacBaohiem`;
    return this.http.post<any>(url, item, { headers: this.httpUtils.getHTTPHeaders() });
  }

  updatePartner(item: InsurancePartnerModel): Observable<any> {
    const url = `${API_INSURANCE_PARTNER}/updateDoitacBaohiem`;
    return this.http.post<any>(url, item, { headers: this.httpUtils.getHTTPHeaders() });
  }

  delete(id: number): Observable<any> {
    const url = `${API_INSURANCE_PARTNER}/DoitacBaohiem/${id}`;
