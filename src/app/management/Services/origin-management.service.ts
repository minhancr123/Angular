...
import { XuatXuModel } from "../Model/Inventory-management.model";

const API_XUAT_XU_URL = `${environment.ApiRoot}/InventoryManagement`;
...
/**
  ...
  getAll(): Observable<XuatXuModel[]> {
    ...
    return this.findData(state).pipe(
      map((res: ResultModel<XuatXuModel>) => res?.data || []),
      catchError(() => of([]))
    );
  }
  ...
  private findData(state: ITableState): Observable<any> {
    ...
    const url = `${API_XUAT_XU_URL}/GetAllOrigin`;
    return this.http.post<any>(url, state, { headers });
  }

  create(xuatXu: XuatXuModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = `${API_XUAT_XU_URL}/addOrigin`;
    return this.http.post<any>(url, xuatXu, { headers: httpHeaders });
  }

  update(xuatXu: XuatXuModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = `${API_XUAT_XU_URL}/updateOrigin`;
    return this.http.post<any>(url, xuatXu, { headers: httpHeaders });
  }

  delete(id: number): Observable<any> {
    const url = `${API_XUAT_XU_URL}/Origin/${id}`;
    const headers = this.httpUtils.getHTTPHeaders();
