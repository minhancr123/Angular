...
const API_DVT_URL = `${environment.ApiRoot}/InventoryManagement`;
...
export class DVTService {
  ...
  getAll(): Observable<DVTModel[]> {
    ...
    return this.findData(state).pipe(
      map((res: ResultModel<DVTModel>) => res?.data || []),
      catchError(() => of([]))
    );
  }
  ...
  private findData(state: ITableState): Observable<any> {
    ...
    const url = `${API_DVT_URL}/GetAllDVT`;
    return this.http.post<any>(url, state, { headers });
  }

  create(dvt: DVTModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = `${API_DVT_URL}/addDVT`;
    return this.http.post<any>(url, dvt, { headers: httpHeaders });
  }

  update(dvt: DVTModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = `${API_DVT_URL}/updateDVT`;
    return this.http.post<any>(url, dvt, { headers: httpHeaders });
  }

  delete(id: number): Observable<any> {
    const url = `${API_DVT_URL}/deleteDVT/${id}`;
    const headers = this.httpUtils.getHTTPHeaders();
