...
import { AssetReasonModel, AssetTypeModel, GroupAssetModel } from '../Model/asset-management.model';

const API_REASON_ASSET_TYPE = `${environment.ApiRoot}/PropertyManagement`;
...
export class ReasonAssetService {
  private http = inject(HttpClient);
  private httpUtils = inject(HttpUtilsService);

  public Visible: boolean;
  public HasPermission: boolean;
  private _items$ = new BehaviorSubject<AssetReasonModel[]>([]);
  ...
  fetchForGuard(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.findData(this._tableState$.value)
        .pipe(
          tap((res: ResultModel<AssetReasonModel>) => {
            if (res && res.status === 1) {
