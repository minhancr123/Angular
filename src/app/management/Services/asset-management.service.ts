...
import { AssetTypeModel } from '../Model/asset-management.model';

const API_ASSET_TYPE = `${environment.ApiRoot}/PropertyManagement`;
...
export class AssetTypeService {
  ...
  private _items$ = new BehaviorSubject<AssetTypeModel[]>([]);
  ...
  fetchForGuard(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.findData(this._tableState$.value)
        .pipe(
          tap((res: ResultModel<AssetTypeModel>) => {
            if (res && res.status === 1) {
