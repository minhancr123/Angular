...
import { LoListService } from '../Services/kho-management.service';
...
export class CustomerManagementListComponent implements OnInit, OnDestroy {
  ...
  displayedColumns =  ['MaLo', 'TenLo', 'NgayNhap', 'GhiChu']; 
  ...
  constructor(
    ...
    public datepipe: DatePipe,
     public loListService: LoListService,
  ) {}

  ngOnInit(): void {
  
    this.loListService.fetch();
  this.paginator = this.loListService.paginator;
  this.sorting = this.loListService.sorting;
  this.grouping = this.loListService.grouping;

  const losb = this.loListService.isLoading$.subscribe(res => this.isLoading = res);
  this.subscriptions.push(losb);
    this.getDateFilter();
...
format_date(value: any): any {
  ...
  if (parts.length < 6) return value; // Nếu không đủ thông tin, trả về nguyên giá trị
  const day = parseInt(parts[0], 10);
  const month = parseInt(parts[1], 10) - 1; // JS tháng tính từ 0
  const year = parseInt(parts[2], 10);
  const hour = parseInt(parts[3], 10);
  const minute = parseInt(parts[4], 10);
  const second = parseInt(parts[5], 10);
  const dateObj = new Date(year, month, day, hour, minute, second);
  if (isNaN(dateObj.getTime())) return value; // Nếu vẫn lỗi, trả lại nguyên giá trị
  return this.datepipe.transform(dateObj, 'dd/MM/yyyy');
}

  paginate(paginator: PaginatorState) {
   this.loListService.patchState({ paginator });
  }
